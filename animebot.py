...
API_TOKEN = '1697552707:AAHYPCaR-dhHaHxC7GQNziLkgGoo_lYBhoI'
#----------------------------------
...
lastcmd = {}
parse_data = {'commands':['/anime', '/hentai', '/loli', '/yuri', '/ecchi', '/neko', '/uncensored', '/wallpaper'], 'tags':['rating:s', 'rating:e', 'loli', 'yuri', 'rating:q', 'cat_ears', 'uncensored', 'wallpaper'], 'ch_id': ['1', '2', '3', '4', '5', '6', '7', '8'], 'chan':['', '@hentai_channel', '@hentai_channel', '@yuri_channel', '@channel_ecchi', '', '@uncensored_channel', ''], 'items':['anime', 'hentai', 'loli', 'yuri', 'ecchi', 'neko', 'uncensored', 'wallpaper'], 'pages':[7658, 1527, 635, 252, 5622, 402, 367, 523]}
...
async def commands(message=None, chat_id=None, chan=None, data=None):
	...
	if data == None:
		data = message.text
		if '@NekobinRobot' in data:
			data = data.replace('@NekoidBot', '')
		try:
			data = data.split(' ', 1)[0]
		except:
			pass
...
async def idd(message, tags=None):
	...
	try:
		...
		posts = client.post_list(tags=tags)
		for post in posts:
			urllib.request.urlretrieve(post['file_url'], "tmp/NekobinRobot_" + str(randomint) + ".jpg")
		try:
...
async def inline_function(inline_query: InlineQuery):
	...
	for post in posts:
		inlinequery.append(InlineQueryResultPhoto(
				...
				caption='<a href="https://t.me/NekobinRobot?start=%s">Download</a>' % (post['id']), 
